      ...
      <div className="w-full max-w-5xl h-[88vh] glass rounded-2xl flex flex-col overflow-hidden shadow-2xl border border-surface-400">
              ...
              {pipelines.map(p => (
                <div
                  ...
                  className={clsx(
                    ...
                    PIPELINE_COLORS[p.name] ?? 'text-slate-400 bg-surface-600 border-surface-500',
                  )}
                >
                  <span>{p.name}</span>
        ...
        <div className="flex border-b border-surface-500 flex-shrink-0 overflow-x-auto scrollbar-hide">
          {outputTabs.map(t => (
            ...
            >
              {t === 'summary' ? '📊 Summary' : `${AGENT_ICON[t] ?? '📄'} ${t}`}
            </button>
          ))}
        </div>
                    ...
                    {outputEntries.map(o => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-base">{AGENT_ICON[o.agent] ?? '📄'}</span>
              ...
              return (
                <div className="space-y-3">
                  <div className="flex items-center gap-2 pb-2 border-b border-surface-600">
                    <span className="text-xl">{AGENT_ICON[output.agent] ?? '📄'}</span>
